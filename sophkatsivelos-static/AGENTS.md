...
- Map the eight home-page targets to Decay, Deficit, Digital Sociology Study, Stories on Skin, Merimnao, Biotic Gallery, Witches' Flight 3D, and MCAAD × Ellwas. Keep Digital Sociology Study on the central target and slightly larger than the other map nodes; keep MCAAD × Ellwas on the prominent lower-left loop target at `13.4%, 75.5%`.
...
- Style hosted-video play controls as hybrid analog-digital transport switches: keep their tactile rectangular form and mechanical depth, then add green phosphor accents, fine signal texture, and coded labels; keep poster previews in color and avoid circular digital beacon buttons.
...
- Preserve the Archive's original mounted-drive anatomy at desktop and laptop widths: a vertical volume sidebar, file browser, and directly adjacent selected-record preview in a compact viewport-bounded stage. Only stack the preview below the browser in the dedicated mobile layout.
- Use cyan, not blue, for Archive project-row hover and keyboard-focus signals.
...
- Keep Deficit's 3D credits compact and collapsed by default within the bottom project-links area, with separate credit groups for website object-field models and models used in the original VR simulation.
...
- Treat pages 63–103 of `MCAAD-Art-Direction.pdf` as the source of truth for Sophie's MCAAD × Ellwas development story: point-to-fan/star light motion, alternate growth languages, dandelion selection and observation, density/movement/color variables, the four-pillar composite, terminal interaction, possible mobile handoff, and interface applications. Explain that the exhibits, people, stories, milestones, and four themes visitors engaged with shaped the light. The physical museum terminal is the primary interaction; mobile is only a possible supporting extension. Preserve the live interactive bloom, begin the process directly with the five development chapters without a separate design-premise overview block, place `Light in Motion` directly between `Finding a Language for Growth` and `Choosing the Dandelion`, keep its readout at `02 MOTION STUDIES`, show only the bloom animation in that section, keep pages 75 and 80–83 together in one wide cycling composite-refinement frame, keep pages 87–90 together in a separate cycling terminal/mobile frame, keep only page 101's in-museum interface in the final application section, keep process and application images inline without expanded-detail behavior, avoid repeated image sources, and do not add the unrelated Dana/OBS GIF treatment.
